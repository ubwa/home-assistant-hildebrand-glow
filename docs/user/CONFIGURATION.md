...
This document describes all configuration options and settings available in the Hildebrand Glow Energy Monitor custom integration.
...
### `hildebrand_glow.example_service`
...
```yaml
service: hildebrand_glow.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: hildebrand_glow.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Hildebrand Glow Energy Monitor Alert
...
- [GitHub Issues](https://github.com/ubwa/home-assistant-hildebrand-glow/issues) - Report problems
